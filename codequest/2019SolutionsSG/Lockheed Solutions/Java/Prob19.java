...
public class Prob19 {
	...
	public static void main(String[] args) throws Exception {
		try (BufferedReader input = new BufferedReader(new InputStreamReader(new FileInputStream(new File("Prob19.in.txt"))))) {
			int testCases = Integer.parseInt(input.readLine());

			for (int testcase = 0; testcase < testCases; testcase++) {
				int numAddresses = Integer.parseInt(input.readLine());
				
				byte[][] binaries = new byte[numAddresses][];
				
				for(int i = 0; i < numAddresses; i++) {
					binaries[i] = convertToBinary(input.readLine());
				}
